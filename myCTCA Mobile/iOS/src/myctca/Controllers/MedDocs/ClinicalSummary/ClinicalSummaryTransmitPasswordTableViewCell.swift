...
class ClinicalSummaryTransmitPasswordTableViewCell: UITableViewCell, UITextFieldDelegate {
    ...
    @IBAction func toggelEyeTapped(_ sender: Any) {
        ...
        if (self.passwordIsVisible == false) {
            self.toggleEyeButton.setImage(#imageLiteral(resourceName: "eyeslash") , for: .normal)
        } else {
            self.toggleEyeButton.setImage(#imageLiteral(resourceName: "eye") , for: .normal)
        }
    }
}
