...
typedef NS_ENUM(NSInteger, ACSHandleType)
{
    ...
    ACSHandleTypeRecordingFeature = 4,
    ACSHandleTypeTranscriptionFeature = 5
...
@class ACSRecordingFeature;
@class ACSTranscriptionFeature;
...
@protocol ACSRecordingFeatureDelegate;
@protocol ACSTranscriptionFeatureDelegate;
...
/**
 * A set of methods that are called by ACSRecordingFeature in response to important events.
 */
NS_SWIFT_NAME(RecordingFeatureDelegate)
@protocol ACSRecordingFeatureDelegate <NSObject>
@optional
- (void)onIsRecordingActiveChanged:(ACSRecordingFeature * _Nonnull)recordingFeature :(ACSPropertyChangedEventArgs * _Nonnull)args NS_SWIFT_NAME( recordingFeature(_:didChangeRecordingState:));
@end

/**
 * A set of methods that are called by ACSTranscriptionFeature in response to important events.
 */
NS_SWIFT_NAME(TranscriptionFeatureDelegate)
@protocol ACSTranscriptionFeatureDelegate <NSObject>
@optional
- (void)onIsTranscriptionActiveChanged:(ACSTranscriptionFeature * _Nonnull)transcriptionFeature :(ACSPropertyChangedEventArgs * _Nonnull)args NS_SWIFT_NAME( transcriptionFeature(_:didChangeTranscriptionState:));
@end
...
-(ACSRemoteParticipant* _Nonnull)addParticipant:(id<CommunicationIdentifier> _Nonnull)participant NS_SWIFT_NAME( add(participant:));
-(ACSRemoteParticipant* _Nonnull)addParticipant:(PhoneNumberIdentifier* _Nonnull) participant options:(ACSAddPhoneNumberOptions* _Nullable)options NS_SWIFT_NAME( add(participant:options:));
-(id _Nonnull)api: (Class _Nonnull)cls NS_REFINED_FOR_SWIFT;
...
/// CallFeature super type, locator for joining meetings
...
NS_SWIFT_NAME(RecordingFeature)
@interface ACSRecordingFeature : ACSCallFeature
-(nonnull instancetype)init NS_UNAVAILABLE;
/// Indicates if recording is active in current call
@property (readonly) BOOL isRecordingActive;

/**
 * The delegate that will handle events from the ACSRecordingFeature.
 */
@property(nonatomic, assign, nullable) id<ACSRecordingFeatureDelegate> delegate;

@end

/// Call Feature for managing call transcription
NS_SWIFT_NAME(TranscriptionFeature)
@interface ACSTranscriptionFeature : ACSCallFeature
-(nonnull instancetype)init NS_UNAVAILABLE;
/// Indicates if transcription is active in current call
@property (readonly) BOOL isTranscriptionActive;

/**
 * The delegate that will handle events from the ACSTranscriptionFeature.
 */
@property(nonatomic, assign, nullable) id<ACSTranscriptionFeatureDelegate> delegate;
