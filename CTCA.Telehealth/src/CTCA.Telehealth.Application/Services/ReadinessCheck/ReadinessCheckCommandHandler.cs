...
{
    public class ReadinessCheckCommandHandler : Common.RequestHandler<ReadinessCheckCommand, ObjectResponse<TelehealthAppointment>>
    {
        ...
        public ReadinessCheckCommandHandler(IMediator mediator, IMapper mapper, ILogger<ReadinessCheckCommandHandler> logger, IMeetingProvider meetingProvider, IMailProvider mailProvider,
        ...
        /// </summary>
        /// <returns></returns>
        protected override async Task<ObjectResponse<TelehealthAppointment>> HandleRequest(ReadinessCheckCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    request.ReadinessCheckRequest.CreatedDT = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, TimeZoneInfo.FindSystemTimeZoneById("Central Standard Time"));
                    request.ReadinessCheckRequest.ModifiedDT = request.ReadinessCheckRequest.CreatedDT;
                }
                else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                {
                    request.ReadinessCheckRequest.CreatedDT = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, TimeZoneInfo.FindSystemTimeZoneById("America/Chicago"));
                    request.ReadinessCheckRequest.ModifiedDT = request.ReadinessCheckRequest.CreatedDT;
                }
                else
                {
                ...
                telAppt = await _meetingProvider.ScheduleReadinessCheckAsync(request.ReadinessCheckRequest, cancellationToken);
                ...
                if (string.IsNullOrEmpty(o365Response.Item1.CalendarID))
                {
                    ...
                    Logger.LogTrace($"Running {request.ReadinessCheckRequest.PatientEmail} against EWS");
            ...
            {
                Logger.LogError($"EntityAlreadyExistsException Faced: {ex.Message}\r{request.ReadinessCheckRequest.Service.SourceServiceId}", System.Reflection.MethodBase.GetCurrentMethod().Name, DateTime.UtcNow.ToLongTimeString());
            ...
            {
                Logger.LogError($"WebException Faced: {ex.Message}\r{request.ReadinessCheckRequest.Service.SourceServiceId}", System.Reflection.MethodBase.GetCurrentMethod().Name, DateTime.UtcNow.ToLongTimeString());
